    ...
    'w: 'a,
{
    ...
    pub fn try_serialize<R: Resource + Serialize>(&mut self, id: Id) -> Result<(), S::Error> {
...
impl<'w, ResId: SerKey, CmpId: SerKey, W: WorldSerdeInstructions<ResId, CmpId>> Serialize
    for ResourcesSerWrapper<'w, ResId, CmpId, W>
{
...
impl<'a, 'de, M: MapAccess<'de>, Id: SerKey> ResourceDeContext<'a, 'de, M, Id> {
    fn new(map: M, key: Id, resources: &'a mut BTreeMap<TypeIdWrapper, Box<dyn Resource>>) -> Self {
        Self {
            map,
            key,
            resources,
    ...
    pub fn accept<R: Resource + DeserializeOwned>(&mut self) -> Result<(), M::Error> {
...
impl<'w, 'de, ResId: SerKey, CmpId: SerKey, W: WorldSerdeInstructions<ResId, CmpId>> Visitor<'de>
    for ResourcesDeWrapper<'w, ResId, CmpId, W>
where
    'de: 'w,
{
    type Value = BTreeMap<TypeIdWrapper, Box<dyn Resource>>;
